"use strict";
//Task 1
const alien_color_green = 'green';
if (alien_color_green === 'green') {
    console.log("The player earned 5 points for shooting the green alien.");
}
else if (alien_color_green === 'yellow') {
    console.log("The player earned 10 points for shooting the yellow alien.");
}
else if (alien_color_green === 'red') {
    console.log("The player earned 15 points for shooting the red alien.");
}
//Task2
const alien_color_yellow1 = 'yellow';
if (alien_color_yellow1 === 'green') {
    console.log("The player earned 5 points for shooting the green alien.");
}
else if (alien_color_yellow1 === 'yellow') {
    console.log("The player earned 10 points for shooting the yellow alien.");
}
else if (alien_color_yellow1 === 'red') {
    console.log("The player earned 15 points for shooting the red alien.");
}
//Task3
const alien_color_red1 = 'red';
if (alien_color_red1 === 'green') {
    console.log("The player earned 5 points for shooting the green alien.");
}
else if (alien_color_red1 === 'yellow') {
    console.log("The player earned 10 points for shooting the yellow alien.");
}
else if (alien_color_red1 === 'red') {
    console.log("The player earned 15 points for shooting the red alien.");
}
